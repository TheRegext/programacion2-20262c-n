...
bool ArchivoLibro::mostrarRegistroLibro(int estado)
{
    /// estado -> 0 Muestra todos
    /// estado -> 1 Muestra activos
    /// estado -> 2 Muestra inactivos
    ...
    while(fread(&reg,sizeof reg, 1, pLibro)==1)
    {
        ...
        ///mostrar el registro segun parametro estado
        ...
        /* if(estado && reg.getEstado()){
             reg.mostrar();
         }
         else if(!estado && !reg.getEstado())
         {
             reg.mostrar();
         }
         */
...
int ArchivoLibro::contarRegistros()
{
    ...
    ///3. contar registros
    fseek(pLibro, 0,SEEK_END); /// se mueve el puntero al final del archivo
    int cantidadBytes = ftell(pLibro);
    int cantidadRegistros = cantidadBytes / sizeof(Libro);
    fclose(pLibro);
    ///4. devolver cantidad de registros
